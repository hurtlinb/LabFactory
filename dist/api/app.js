...
const computeLockKey = (input) => {
    if (input.lockKey)
        return input.lockKey;
    if (input.type === 'terraform_apply' || input.type === 'terraform_plan') {
        return `${input.project}:${input.environment}:terraform`;
    }
    const inventory = typeof input.payload?.inventory === 'string' && input.payload.inventory.length > 0
...
export const createApiApp = ({ executionRepo, schedulerQueue }) => {
    const app = express();
    app.use(express.json());
    const executionIdFrom = (req) => Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    app.post('/executions', wrapAsync(async (req, res) => {
        ...
        await schedulerQueue.add('schedule', { executionId: execution.id }, { removeOnComplete: true, attempts: 5, backoff: { type: 'exponential', delay: 3000 } });
        res.status(202).json({ executionId: execution.id });
    }));
    app.get('/executions/:id', wrapAsync(async (req, res) => {
    ...
    app.get('/executions/:id/logs', wrapAsync(async (req, res) => {
        const executionId = executionIdFrom(req);
        const execution = await executionRepo.findById(executionId);
        if (!execution) {
            res.status(404).json({ error: 'execution not found' });
    ...
    app.post('/executions/:id/cancel', wrapAsync(async (req, res) => {
        const executionId = executionIdFrom(req);
