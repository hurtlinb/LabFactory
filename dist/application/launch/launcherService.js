...
export class LauncherService {
    constructor(executionRepo, lockService, jobClient = new KubernetesJobClient()) {
    ...
    async waitForJobCompletion(jobName, executionId) {
        ...
        while (true) {
            ...
            if (status.failed && status.failed > 0) {
                const failedCondition = status.conditions?.find((c) => c.type === 'Failed');
                return {
                    succeeded: false,
    ...
    async ingestRunnerLogs(executionId, jobName) {
        try {
