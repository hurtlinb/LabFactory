const terraformSettingDefinitions = {
  ...
  proxmox_parallel: 'number',
  proxmox_nodes: 'stringList'
};
...
const optionalTerraformEnvKeys = new Set(['proxmox_nodes']);
const requiredTerraformEnvKeys = Object.keys(terraformEnvMapping).filter(
  key => !optionalTerraformEnvKeys.has(key)
);
...
export const readTerraformEnvSettings = () => {
  ...
  for (const [settingKey, envVarName] of Object.entries(terraformEnvMapping)) {
    ...
    } else if (terraformEnvValueTypes[settingKey] === 'stringList') {
      result[settingKey] = parseStringListValue(normalized);
    } else {
