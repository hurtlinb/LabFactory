...
export class SchedulerService {
  constructor(
    ...
    private schedulerQueue: QueuePort,
    private launcherQueue: QueuePort
  ) {}
  ...
  async handle(job: Job) {
    ...
    if (hasConflict) {
      ...
      await this.schedulerQueue.add(
        'schedule',
        { executionId },
        { delay: lockConfig.retryDelayMs, removeOnComplete: true, removeOnFail: true }
      );
      return;
    }
