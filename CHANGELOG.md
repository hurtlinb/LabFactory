...
- **Critique** — Constaté sur un lab avec beaucoup de VM Win11 (boot/sysprep lent) : même avec le fix ci-dessus, chaque round se faisait quand même tuer par le timeout (mesuré à 3min30 = 180s + 30s de backoff, en boucle, pendant 20+ minutes). Cause : les timeouts WinRM par VM (`operation_timeout_sec: 30` / `read_timeout_sec: 45`) venaient de l'ancien modèle "1 VM à la fois" — une seule VM dont la connexion WinRM traîne (port ouvert mais service pas encore pleinement opérationnel pendant sysprep) suffit à empêcher Ansible d'atteindre la tâche de marquage pour **tout le round**, et notre code réinitialisait alors la progression de confirmation de chaque VM à chaque round raté. Réduits à 8s/12s (un essai à 3s/5s s'est révélé trop agressif : il augmentait les collisions "resource is in use" côté WinRM, chacune coûtant jusqu'à 15s via une boucle de retry interne à Ansible)
- **Critique** — Cause racine du dépassement de timeout par round, plus impactante que les deux points ci-dessus : le check de disponibilité Windows lançait `ansible-playbook` avec le dossier `terraform/` comme répertoire de travail, qui ne contient pas `ansible.cfg` (`forks = 100`, défini dans `ansible/`) — Ansible retombait donc sur sa valeur par défaut (`forks = 5`). Avec ~66 VM dans le lot, ça veut dire ~14 lots séquentiels de 5 au lieu d'un seul lot parallèle, dépassant largement les 180s peu importe le réglage des timeouts par hôte. Corrigé en exécutant depuis `ansible/`, comme le fait déjà le check de reconnexion post-redémarrage
- Le check de disponibilité Windows exigeait 2 rounds consécutifs réussis avant de confirmer une VM prête (hérité de l'ancien modèle par VM). En lot, cette exigence double l'exposition aux VM lentes du lot et n'apporte plus grand-chose vu la robustesse du check lui-même (vérifie le flag de fin + sa fraîcheur + l'arrêt du service) — un seul round réussi suffit désormais, comme pour le check de reconnexion post-redémarrage
- **Critique** — Même après les fixes ci-dessus, un round contenant au moins une VM très lente pouvait encore se faire tuer à 180s, et dans ce cas **aucune** VM de ce round n'était confirmée prête — y compris celles ayant réussi le check en quelques secondes. Cause : la stratégie par défaut d'Ansible (`linear`) bloque toutes les VM du round à la tâche de vérification jusqu'à ce que la plus lente ait fini, avant de laisser quiconque passer à la tâche d'écriture du marqueur. Passé en stratégie `free` : chaque VM progresse indépendamment des autres et écrit son marqueur dès qu'elle est prête, sans attendre les VM à la traîne du même round
